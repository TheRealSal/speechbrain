...
class Separation(sb.Brain):
    ...
    def compute_forward(self, mix, targets, stage, noise=None):
        ...
        #############################
        # Run Mix Through Teacher(Pretrained) Model #
        #############################
        ...
        return est_source, targets, targets_transmitted, mix_s, targ_w, mix_sq, targ_q, mix_sq_codes, targ_q_codes, teacher_est_source

    def compute_objectives(self, predictions, targets, mix_s, targ_w, mix_sq, targ_q, mix_sq_codes, targ_q_codes,
                           stage, teacher_predictions=None):
        """Computes the sinr loss"""
        if stage == sb.Stage.TRAIN:
            #Add teacher loss
            soft_loss = self.hparams.loss(teacher_predictions, predictions)

            hard_loss = self.hparams.loss(targets,
                                          predictions)  # modify the ground-truth accordingly if you would like to train on the cSI-SDR loss

            alpha = self.hparams.loss_weight
        ...
        with self.no_sync(not should_step):
            if self.use_amp:
                with torch.autocast(
                    ...
                    predictions, targets, targets_transmitted, mix_s, targ_w, mix_sq, targ_q, mix_sq_codes, targ_q_codes, teacher_predictions = self.compute_forward(
                        mixture, targets, sb.Stage.TRAIN
                    )
                    loss = self.compute_objectives(predictions, targets, mix_s, targ_w, mix_sq, targ_q, mix_sq_codes,
                                                   targ_q_codes, sb.Stage.TRAIN, teacher_predictions)
    ...
    def evaluate_batch(self, batch, stage):
        ...
        with torch.no_grad():
            predictions, targets, targets_transmitted, mix_s, targ_w, mix_sq, targ_q, mix_sq_codes, targ_q_codes, _ = self.compute_forward(
        ...
        with open(save_file, "w") as results_csv:
            ...
            with tqdm(test_loader, dynamic_ncols=True) as t:
                for i, batch in enumerate(t):
                    ...
                    # Compute SI-SNR
                    sisnr = self.compute_objectives(predictions, targets, mix_s, targ_w, mix_sq, targ_q, mix_sq_codes,
                                                    targ_q_codes, sb.Stage.TEST)
                    Csisnr = self.compute_objectives(predictions, targets_transmitted, mix_s, targ_w, mix_sq, targ_q,
                                                     mix_sq_codes, targ_q_codes, sb.Stage.TEST)
                    ...
                    sisnr_baseline = self.compute_objectives(mixture_signal, targets, mix_s, targ_w, mix_sq, targ_q,
                                                             mix_sq_codes, targ_q_codes, sb.Stage.TEST)
                    Csisnr_baseline = self.compute_objectives(mixture_signal, targets_transmitted, mix_s, targ_w,
                                                              mix_sq, targ_q, mix_sq_codes, targ_q_codes, sb.Stage.TEST)
