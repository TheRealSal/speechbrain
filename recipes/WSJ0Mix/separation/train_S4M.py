...
class Separation(sb.Brain):
        ...
        with self.no_sync(not should_step):
            if self.use_amp:
                with torch.autocast(
                    ...
                    predictions, targets, teacher_predictions = self.compute_forward(
                        mixture, targets, sb.Stage.TRAIN
                    )
                    loss = self.compute_objectives(predictions, targets, teacher_predictions)
    ...
    def evaluate_batch(self, batch, stage):
        ...
        with torch.no_grad():
            predictions, targets, _ = self.compute_forward(mixture, targets, stage)
        ...
        with open(save_file, "w", newline="", encoding="utf-8") as results_csv:
            ...
            with tqdm(test_loader, dynamic_ncols=True) as t:
                for i, batch in enumerate(t):
                    ...
                    with torch.no_grad():
                        predictions, targets, _ = self.compute_forward(
